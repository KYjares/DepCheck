...
class Assessment extends Component {
    ...
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        db.collection("Users")
          ...
          .doc(Moment(new Date().getTime()).format("YYYY-MM-DD HH:mm:ss"))
          .set({
            depressionCategory: this.calculateDepressionRisk()
          });
      }
    });
  };
  ...
  //     .ref(`/BIR/${this.state.upload.name}`)
  //     .put(this.state.upload);
  //   uploadTask.on(
  //     "state_changed",
  //     snapShot => {
...
const WrappedAssessment = compose(
  withBase,
  withRouter,
  withFirebase
)(Assessment);
